  ...
  >
    ...
    <template v-for="(resource, index) in resources">
        <tr
            ...
            @collapse-relations="collapseRelations"
        ...
        />
        <tr class="content" v-if="visibleRelations.includes(resource.id.value)
          && resource.hasOwnProperty('relatedResources')
          && resource.relatedResources.length > 0">
          <td :colspan="fields.length + 2" class="p-0">
            <collapse-resources
              v-for="(relatedResource, index) in resource.relatedResources"
              :key="index"
              :field="relatedResource.field"
              :resource-name="relatedResource.field.resourceName"
              :via-resource="relatedResource.resourceName"
              :via-resource-id="resource.id.value"
              :via-relationship="relatedResource.relationship"
              :relationship-type="relatedResource.relationshipType"
              :initialPerPage="relatedResource.field.perPage || 5"
              :shouldOverrideMeta="false"
            >
            </collapse-resources>
          </td>
        </tr>
...
export default {
  ...
  methods: {
    ...
    collapseRelations(data) {
      if (data.collapse) {
          this.visibleRelations.push(data.resource)
      } else {
          this.visibleRelations.splice(this.visibleRelations.indexOf(data.resource), 1)
      }
    },
  },
