  ...
  <tr :dusk="resource['id'].value + '-row'">
    ...
    <td class="td-fit text-right pr-6 align-middle">
      <div class="inline-flex items-center">
        ...
        <button
            class="inline-flex appearance-none cursor-pointer text-70 hover:text-primary mr-3"
            @click.prevent="collapseRelations"
            v-if="hesCollapseRelations">

            <icon v-if="!collapseOpen" type="arrow-down" width="20" height="21" view-box="0 0 451.847 451.847"/>
            <icon v-else type="arrow-down" style="transform: rotate(180deg);" width="20" height="21" view-box="0 0 451.847 451.847"/>
        </button>
...
export default {
  ...
  methods: {
    ...
    collapseRelations () {
      ...
      this.$emit('collapse-relations', {
         collapse: this.collapseOpen,
         resource: this.resource.id.value,
      })
    }
  },
  ...
  computed: {
    ...
    hesCollapseRelations() {
      return this.resource.hasOwnProperty('relatedResources')
      && this.resource.relatedResources.length > 0
    }
  },
}
</script>
